...
#include <signal.h>
#define BACKLOG 10
#define SIDTPORT "41010"
#define SIDFPORT "22010"
...
int phase3(){
	...
	printf("The File Server 1 has TCP port number %s",SIDTPORT);
	...
	while(1) { // main accept() loop
		...
		if (!fork()) { // this is the child process
			...
			clientName=pc;
			printf("File server 1 received request from %s, ",pc);
			pc = strtok (NULL, " ");
...
int main()
{
	...
	char *mes="File_Server1 41010";

	struct addrinfo hints,hintsTcp, *servinfo, *servTcp, *p,*servFile,*res;
	int rv,rs;
	int y=1;
	...
	struct hostent *lh = gethostbyname("nunki.usc.edu");
	printf("The File Server 1 has UDP port number %s",SIDFPORT);
	...
	printf("The registration request from FileServer1 has been sent to the Directory Server.\n ");
	printf("End of Phase 1 for File Server1.\n ");
	close(sockfd);
	freeaddrinfo(servinfo); // all done with this structure
