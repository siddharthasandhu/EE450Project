...
#include <signal.h>
#define BACKLOG 10
#define SIDTPORT "42010"
#define SIDFPORT "23010"
...
int phase3(){
	...
	printf("The File Server 2 has TCP port number %s",SIDTPORT);
	...
	while(1) { // main accept() loop
		...
		if (!fork()) { // this is the child process
			...
			clientName=pc;
			printf("File server 2 received request from %s, ",pc);
			pc = strtok (NULL, " ");
			printf(" for the file %s.\n ",pc);
			docName=pc;
			if (send(new_fd, docName, 20, 0) == -1)
				perror("send");
			printf("File server 2 has sent %s, ",docName);
			printf(" to %s.\n ",clientName);
			close(new_fd);
			exit(0);
		}
...
int main()
{
	...
	struct hostent *lh = gethostbyname("nunki.usc.edu");
	printf("The File Server 2 has UDP port number %s",SIDFPORT);
	printf(" and IP address  %s .\n",lh->h_name);
	...
	printf("The registration request from FileServer2 has been sent to the Directory Server.\n ");
	printf("End of Phase 2 for File Server 2.\n ");
	close(sockfd);
	freeaddrinfo(servinfo); // all done with this structure
