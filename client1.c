...
int phase3(char buf1[]){
	...
	pch = strtok (buf1," ");
	fileServName=pch;
	printf("The request by Client 1 is present in %s ",pch);
	pch = strtok (NULL, " ");
	strcpy(PORTNAME,pch);
	PORTNAME[5]='\0';
	...
	printf("Client 1 received %s", buf);
	printf("from %s . \n",fileServName);
	printf("End of Phase 3 for Client 1.\n");
...
int main()
{
	...
	printf("The Client 1 has UDP port number %s",SIDFPORT);
	...
	printf("The file request from Client 1 has been sent to the Directory Server.\n");
	if ((numbytes = recv(sockfd, buf, MAXDATASIZE-1, 0)) == -1) {
		perror("recv");
		exit(1);
	}
